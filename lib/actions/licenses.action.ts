...
import { deleteLicenseSchema, licensesSchema, updateLicenseSchema, updateLicenseSignatureSchema, updateLicenseStatusSchema } from "@/types/license-schema";
import { eq } from "drizzle-orm";




export const RegisterLicense = actionClient
  ...
  .action(
    async ({ parsedInput: {
      ...
      function generateLicenseRefId(): string {
        ...
        const month = String(now.getMonth() + 1).toString().padStart(2, "0"); // 05
...
export const UpdateLicense = actionClient.schema(updateLicenseSchema).action(async ({ parsedInput }) => {
  try {
    ...
    return { error: `Failed to update license: ${error instanceof Error ? error.message : String(error)}` }
  }
})
...
export const DeleteLicense = actionClient
  ...
  .action(async ({ parsedInput: { license_ref_id } }) => {
    try {
      ...
      await db.delete(licenses).where(eq(licenses.license_ref_id, license_ref_id));
...
export const UpdateLicenseStatus = actionClient
  ...
  .action(async ({ parsedInput: { id, status } }) => {
    try {
      ...
      const statusText = status === "PENDING" ? "pending" : status === "APPROVED" ? "approved" : "removed"
      return { success: `License ${statusText} successfully` }
    } catch (error) {
      console.error("Error updating license status:", error)
      return { error: `Failed to update license status: ${error instanceof Error ? error.message : String(error)}` }
    }
  })
...
export const UpdateLicenseSignature = actionClient
  ...
  .action(async ({ parsedInput: { id, signature } }) => {
    try {
      ...
      return { error: `Failed to update license signature: ${error instanceof Error ? error.message : String(error)}` }
    }
  })
